    ...
    for cell in ws[1]:  # ws[1] accesses the first row
        for key in col_indices:
            if cell.value and key.lower() in cell.value.lower():
                col_indices[key] = cell.column  # Corrected to 1-based index
        ...
        if target_time - tolerance <= time_value <= target_time + tolerance:
            for i in range(1, 7):  # For each channel
                ...
                data = {
                    ...
                    "Resistance": row[col_indices[resistance_key]].value
                }
...
def fill_sheet2(directory, excel_name, data):
    ...
    for row in range(2, ws.max_row + 1):
        channel_cell = ws.cell(row=row, column=1).value
        temp_cell = ws.cell(row=row, column=2).value
        if channel_cell == data["Channel"] and temp_cell == data["Set Temperature"]:
            for col in range(3, ws.max_column + 1):
                current_cell = ws.cell(row=1, column=col).value
                if current_cell == data["Set Current"]:
                    ws.cell(row=row, column=col, value=data["Resistance"])
                    print(f"Data filled at Row: {row}, Column: {col}")
                    found = True
                    break
        if found:
            break
    ...
    if not found:
        print("No matching row and column found for the data provided.")
